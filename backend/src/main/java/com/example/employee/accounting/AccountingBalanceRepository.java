...
public class AccountingBalanceRepository {
    ...
    public List<AccountingBalance> findByUserId(Long userId) {
        return jdbcTemplate.query("SELECT * FROM ACCOUNTING_BALANCES WHERE USER_ID = ? ORDER BY TYPE ASC, ID ASC", rowMapper, userId);
    }
    ...
    public Optional<AccountingBalance> findByIdAndUserId(Long id, Long userId) {
        return jdbcTemplate.query("SELECT * FROM ACCOUNTING_BALANCES WHERE ID = ? AND USER_ID = ?", rowMapper, id, userId).stream().findFirst();
    }

    public Optional<AccountingBalance> findFirstByUserIdAndAccountName(Long userId, String accountName) {
        return jdbcTemplate.query("SELECT * FROM ACCOUNTING_BALANCES WHERE USER_ID = ? AND ACCOUNT_NAME = ? ORDER BY ID ASC LIMIT 1", rowMapper, userId, accountName)
                .stream().findFirst();
    }
    ...
    public List<AccountingBalance> findByTypeAndRepaymentDay(AccountingBalanceType type, Integer repaymentDay) {
        return jdbcTemplate.query("SELECT * FROM ACCOUNTING_BALANCES WHERE TYPE = ? AND REPAYMENT_DAY = ? ORDER BY ID ASC", rowMapper, type.name(), repaymentDay);
    }

    public List<AccountingBalance> findByUserIdAndTypeAndRepaymentDay(Long userId, AccountingBalanceType type, Integer repaymentDay) {
        return jdbcTemplate.query("""
                SELECT * FROM ACCOUNTING_BALANCES
                 WHERE USER_ID = ? AND TYPE = ? AND REPAYMENT_DAY = ?
                 ORDER BY ID ASC
                """, rowMapper, userId, type.name(), repaymentDay);
    }
    ...
    public void delete(AccountingBalance balance) {
        jdbcTemplate.update("DELETE FROM ACCOUNTING_BALANCES WHERE ID = ?", balance.getId());
    }

    public void deleteByUserId(Long userId) {
        jdbcTemplate.update("DELETE FROM ACCOUNTING_BALANCES WHERE USER_ID = ?", userId);
    }
